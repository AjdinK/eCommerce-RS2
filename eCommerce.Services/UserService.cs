...
public class UserService : IUserService
{
    ...
    public async Task<List<UserResponse>> GetAsync(UserSearchObject search)
    {
        ...
        if (!string.IsNullOrEmpty(search.Username)) query = query.Where(u => u.Username.Contains(search.Username));

        if (!string.IsNullOrEmpty(search.Email)) query = query.Where(u => u.Email.Contains(search.Email));
