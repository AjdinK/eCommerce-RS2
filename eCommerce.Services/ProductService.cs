...
public class ProductService : IProductService
{
    ...
    public async Task<List<ProductResponse>> GetAsync(ProductSearchObject search)
    {
        ...
        if (!string.IsNullOrEmpty(search.Code)) query = query.Where(p => p.SKU.Contains(search.Code));

        if (!string.IsNullOrEmpty(search.CodeGTE)) query = query.Where(p => p.SKU.StartsWith(search.CodeGTE));
